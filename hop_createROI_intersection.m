...
% Checks on subject list are done in the option function, here we assume a
% valid list.
% Folder is picked in the loop

% Iterate over each subject
for iSub = 1:length(m.subjects)

    % Get subject name
    subName = m.subjects{iSub};
    fprintf('\n###### Processing %s ######\n', subName);
    ...
    for iROI = 1:length(m.roisToCreate)
        ...
        % Load ROI image and extract volume data
        [roiImg, roiStruct] = loadROIdata(currROI.roiPath);
        ...
        % Load SPM model 
        [model, loadSuccess] = loadSPMmodel(subjectFolder); % Import the SPM model file
        ...
        checkCorrespondence(roiStruct, tStruct);
...
function [roiImg, roiStruct] = loadROIdata(roiImgPath)
